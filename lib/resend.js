 ...
 */
function createGiftCardEmailTemplate({ recipientName, amountDisplay, code, message, shopUrl }) {
  const greeting = recipientName || 'Friend';
...
<body>
    <div class="container">
        ...
        <div class="content-section">
            ...
            <p class="gift-card-subtitle">You've received a ${amountDisplay} gift card to Katie Ann Clay!</p>
 ...
 */
async function sendGiftCardEmail({ to, recipientName, amountDisplay, code, message, shopUrl }) {
  const resend = getResendClient();
  ...
  try {
    ...
    console.log(`Sent gift card email to ${to} for ${amountDisplay} (code: ...${code.slice(-4)})`);
    return result;

  } catch (error) {
    console.error(`Error sending gift card email to ${to}:`, error);
    throw error;
  }
}
